...
| `rerank_volatile_tolerance_multiplier` | `2.0` | Multiplier on the tolerance when the query is about something that goes out of date (pricing, an API, a regulation, an announcement, or an explicit "latest"/"current" ask — lexicon in `app/retrieval/volatility.py`). Widening lets the recency tie-break fire more often but never lets recency cross a band. `1.0` ranks volatile and stable topics identically |
| `rerank_substance_ratio` | `1.5` | How much more text one passage must hold than another, within a relevance band, before it counts as substantially more complete and leads it. Length is a proxy for completeness (accuracy is not measurable at ranking time). Raise it to make completeness matter less; a very large value hands every relevance tie to recency |
