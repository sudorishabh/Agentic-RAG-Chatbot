...
distinguished "upcoming" from "ever", and nothing consulted an event's own start
date: the only date on a chunk is ``published_at``, which is when the page was
published, not when the event happens. Ranking by ``published_at`` for an
"upcoming" question is close to ranking by the opposite of the answer.

What this can and cannot do
---------------------------
``field_event_start_date`` lives in MySQL ``documents.raw_meta`` and is **not**
in the Qdrant payload, so it cannot be a pre-filter on the vector search without
re-ingesting the corpus. It is therefore applied as a post-retrieval gate: the
