"""Citation assembly (step 7, §8).

The golden rule (§8): citations are built **in code from the retrieved-chunk
payloads**, never generated by the LLM. The model only emits ``[n]`` markers; the
mapping ``n → real source metadata`` happens here, which makes fabricated URLs /
page numbers structurally impossible.

One citation is produced per context block, so a block's number ``n`` is exactly
the marker the LLM was shown — markers always map to a real source (§8.5).
"""
...
def _pdf_link(payload: dict[str, Any]) -> str | None:
    """Deep-link into the PDF viewer at the cited page (§8.2)."""
...
def _source_from_payload(payload: dict[str, Any]) -> CitationSource:
    """Render an alternate/secondary source entry (§9.1)."""
...
def build_citations(blocks: list[ContextBlock]) -> list[Citation]:
    """Build the numbered citation list from the blocks actually sent to the LLM."""
