# Qdrant points — ES2012RS02.pdf

- points (rows upserted): **35**
- embedded: **True** · model `text-embedding-3-large` · dim 3072

Each point is `{id, vector, payload}` exactly as `index_chunks` upserts it. Children carry their embedding; parents carry a zero vector and are reached through their children. Below, vectors are truncated and `chunk_text` is clipped — see `05_qdrant_points.json` for the full data.

---

## Parent · `e7410ead-6b92-5ed8-a3b0-36cbb536d5c6`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "e7410ead-6b92-5ed8-a3b0-36cbb536d5c6",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "chunk_text": "Relative Environment Economics Of Natural Gas and \nOther Fossil Fuels for Power Generation and Policy \nOptions for India\n\nA Petroleum Federation Of India Study\n\nThe Energy and Resources Institute (Knowledge Partner) \nNew Delhi\n\nOverview\n\nBackground \nDescription of the study and findings \nCarbon Implications of power generation \nDomestic coal and gas availability and projected \nrequirements \nInternational fuel markets \nConclusions \nSuggestions – Policy Options \nExclusion – nuclear, renewables, clean coal, hydro\n\nBackground\n\nGrowing demand for power in India\n\nConstraints on supply of f\n\n… [+463 more chars]",
  "content_hash": "1ce4948a88674ecc372d60abe7647375eb2e02d2c0ca228de924da29dc32c694",
  "token_count": 226,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    1,
    3
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `ffcdad11-10ab-50d0-a5e9-3e153801158b`

- vector: dim=3072 · [-0.0217, -0.0175, -0.0101, 0.0101, -0.0123, 0.0030, -0.0387, 0.0084, …]

```json
{
  "chunk_id": "ffcdad11-10ab-50d0-a5e9-3e153801158b",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "chunk_text": "Relative Environment Economics Of Natural Gas and \nOther Fossil Fuels for Power Generation and Policy \nOptions for India\n\nA Petroleum Federation Of India Study\n\nThe Energy and Resources Institute (Knowledge Partner) \nNew Delhi\n\nOverview\n\nBackground \nDescription of the study and findings \nCarbon Implications of power generation \nDomestic coal and gas availability and projected \nrequirements \nInternational fuel markets \nConclusions \nSuggestions – Policy Options \nExclusion – nuclear, renewables, clean coal, hydro\n\nBackground\n\nGrowing demand for power in India\n\nConstraints on supply of f\n\n… [+463 more chars]",
  "content_hash": "1ce4948a88674ecc372d60abe7647375eb2e02d2c0ca228de924da29dc32c694",
  "token_count": 226,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "e7410ead-6b92-5ed8-a3b0-36cbb536d5c6",
  "chunk_index": 0,
  "page_number": 1,
  "page_range": [
    1,
    3
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `8fa3efc2-11cb-5e0c-9b7d-c247d364ce7e`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "8fa3efc2-11cb-5e0c-9b7d-c247d364ce7e",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "India's Power Sector",
  "chunk_text": "India's Power Sector\n\n☐\nInstalled capacity in the country is based largely\non coal\n\n☐\nAddition to capacity has been slower than planned\nwith large gap between target and actual\naddition\n\n☐\nAvailability of domestic fuel - a major factor\nconstraining the production of power in India in the\npast few years\n\nD Stagnating domestic coal production\n☐\n\nDeclining gas production\n☐\n\nA generating loss of 11.7 Billion Units reported up\n☐\nto January 2013 due to shortage of coal supplies\n\nGas shortages have also affected power\n☐\ngeneration\n\nSupply of gas up to January 2013 was 41.45\n☐\nmmscmd as against a requ\n\n… [+246 more chars]",
  "content_hash": "f6d0d4a147de77564e1ed762d9f08ce8daceab0624c2b7d54010289452e2c7ad",
  "token_count": 261,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    4,
    4
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `77036ff1-3d87-5896-a318-46737ff77520`

- vector: dim=3072 · [-0.0198, 0.0048, 0.0110, 0.0400, -0.0228, -0.0142, -0.0320, 0.0327, …]

```json
{
  "chunk_id": "77036ff1-3d87-5896-a318-46737ff77520",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "India's Power Sector",
  "chunk_text": "☐\nInstalled capacity in the country is based largely\non coal\n\n☐\nAddition to capacity has been slower than planned\nwith large gap between target and actual\naddition\n\n☐\nAvailability of domestic fuel - a major factor\nconstraining the production of power in India in the\npast few years\n\nD Stagnating domestic coal production\n☐\n\nDeclining gas production\n☐\n\nA generating loss of 11.7 Billion Units reported up\n☐\nto January 2013 due to shortage of coal supplies\n\nGas shortages have also affected power\n☐\ngeneration\n\nSupply of gas up to January 2013 was 41.45\n☐\nmmscmd as against a requirement of 86 mmscmd\n\n\n\n… [+224 more chars]",
  "content_hash": "4c2872e5ecda9075136c124f69b336e07978c470f2c62a410f75576b11016950",
  "token_count": 256,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "8fa3efc2-11cb-5e0c-9b7d-c247d364ce7e",
  "chunk_index": 1,
  "page_number": 4,
  "page_range": [
    4,
    4
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `0415cb70-a319-563c-af4b-51a72d7b37e9`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "0415cb70-a319-563c-af4b-51a72d7b37e9",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "7th Plan — 8th Plan — 9th Plan — 10th Plan — 11 th Plan — Target — Achieved — The Study",
  "chunk_text": "7th Plan — 8th Plan — 9th Plan — 10th Plan — 11 th Plan — Target — Achieved — The Study\n\nComparison of the cost of power generation using different fuels (i.e. coal \nand gas both domestic and international) across various locations in the \ncountry\n\nIncluding the cost of carbon emissions generated in the process of extraction \nand combustion of fuel\n\nNeed to plan the energy future based on evolving international market \ndynamics of coal and gas while also accounting for the impact of energy on \nclimate\n\nCalculating the Cost of Power Generation \n\nBasic assumptions: \n9 locations across the c\n\n… [+321 more chars]",
  "content_hash": "c2aa9da5de558e631990254af9a1ce18f481e18d14e305fab5cf361494021709",
  "token_count": 228,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    5,
    6
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `070bdaa1-5597-51ba-8f30-5f504bdc3bc1`

- vector: dim=3072 · [-0.0307, -0.0064, -0.0106, 0.0122, -0.0142, -0.0130, -0.0387, 0.0190, …]

```json
{
  "chunk_id": "070bdaa1-5597-51ba-8f30-5f504bdc3bc1",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "7th Plan — 8th Plan — 9th Plan — 10th Plan — 11 th Plan — Target — Achieved — The Study",
  "chunk_text": "Comparison of the cost of power generation using different fuels (i.e. coal \nand gas both domestic and international) across various locations in the \ncountry\n\nIncluding the cost of carbon emissions generated in the process of extraction \nand combustion of fuel\n\nNeed to plan the energy future based on evolving international market \ndynamics of coal and gas while also accounting for the impact of energy on \nclimate\n\nCalculating the Cost of Power Generation \n\nBasic assumptions: \n9 locations across the country \nCombination of pithead, port based \nand inland power stations \nDependence on a \n\n… [+232 more chars]",
  "content_hash": "54feef5a4545e201c055f62c48f90123d5bfaf89c8e74eb35affbf989e7fcb74",
  "token_count": 196,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "0415cb70-a319-563c-af4b-51a72d7b37e9",
  "chunk_index": 2,
  "page_number": 5,
  "page_range": [
    5,
    6
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `50ce9e1c-7bc7-56e0-a984-a6a44b7bd7b9`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "50ce9e1c-7bc7-56e0-a984-a6a44b7bd7b9",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Assumptions — Fixed Costs and O&M — Costs of Fuel",
  "chunk_text": "Assumptions — Fixed Costs and O&M — Costs of Fuel\n\n| Element | Unit | Domestic Coal | Imported Coal | Domestic Gas | LNG |\n| --- | --- | --- | --- | --- | --- |\n| Heat rate | kcal/kWh | 2500 | 2500 | 2000 | 2000 |\n| Capital cost | Rs. million/MW | 44 | 44 | 33 | 33 |\n| PLF | % | 82 | 82 | 72 | 72 |\n| Auxiliary consumption | % | 9.50 | 9.50 | 3 | 3 |\n| Life of plant | Years | 25 | 25 | 25 | 25 |\n| Fixed operating cost as % of capital cost | % | 3.49 | 3.49 | 4.65 | 4.65 |\n| Discount Rate | % | 15.13 | 15.13 | 15.13 | 15.13 |\n\n| Fuel Type | Price* |\n| --- | --- |\n| Domestic Coal | Rs. 1003.42 - \n\n… [+633 more chars]",
  "content_hash": "5b80d579006c3a53c7c532274151c1a91e29541dc35dc1e6e5a0fd9b49947d0b",
  "token_count": 426,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    7,
    7
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `e4edf7d8-9a60-5a8a-bf30-6c06d3f40ae9`

- vector: dim=3072 · [-0.0313, -0.0163, -0.0055, 0.0456, -0.0245, -0.0281, -0.0433, 0.0236, …]

```json
{
  "chunk_id": "e4edf7d8-9a60-5a8a-bf30-6c06d3f40ae9",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Assumptions — Fixed Costs and O&M — Costs of Fuel",
  "chunk_text": "| Element | Unit | Domestic Coal | Imported Coal | Domestic Gas | LNG |\n| --- | --- | --- | --- | --- | --- |\n| Heat rate | kcal/kWh | 2500 | 2500 | 2000 | 2000 |\n| Capital cost | Rs. million/MW | 44 | 44 | 33 | 33 |\n| PLF | % | 82 | 82 | 72 | 72 |\n| Auxiliary consumption | % | 9.50 | 9.50 | 3 | 3 |\n| Life of plant | Years | 25 | 25 | 25 | 25 |\n| Fixed operating cost as % of capital cost | % | 3.49 | 3.49 | 4.65 | 4.65 |\n| Discount Rate | % | 15.13 | 15.13 | 15.13 | 15.13 |\n\n| Fuel Type | Price* |\n| --- | --- |\n| Domestic Coal | Rs. 1003.42 - 3056.02/tonne |\n| Imported Coal | Rs. 6956.46 - 882\n\n… [+582 more chars]",
  "content_hash": "5b4f0f4ba0daab487d43e24eca7f8c2e1edfe5321a17e976f0e774ec0f3d9853",
  "token_count": 412,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "50ce9e1c-7bc7-56e0-a984-a6a44b7bd7b9",
  "chunk_index": 3,
  "page_number": 7,
  "page_range": [
    7,
    7
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `34c69d7e-cc43-5579-9ded-b0bb9767afb8`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "34c69d7e-cc43-5579-9ded-b0bb9767afb8",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Financial Cost of Power Generation",
  "chunk_text": "Financial Cost of Power Generation\n\n\nDomestic coal based generation at pit head power plants is less expensive than \nnon-pit head coal based generation\n\n\nImported coal is not as competitive as domestic coal due to its higher costs\n\n\nDomestic gas based power generation is more expensive than domestic coal based \ngeneration at all locations\n\n\nAt locations away from the pithead, the difference between the cost of power \ngenerated using the two fuels reduces substantially. These are Delhi, Vadodara, \nKochi and Agartala\n\n\nLNG is the most expensive fuel for power generation due to the high cost\n\n… [+108 more chars]",
  "content_hash": "11c159f303f49f8e7a491cc73a443fc3d30f3b549b089fcc94d2ca1ceb7398e5",
  "token_count": 167,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    8,
    9
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `76a9a9f1-e21e-5938-89cc-daa60b51c8fe`

- vector: dim=3072 · [-0.0306, -0.0017, -0.0027, 0.0344, -0.0279, -0.0026, -0.0274, 0.0199, …]

```json
{
  "chunk_id": "76a9a9f1-e21e-5938-89cc-daa60b51c8fe",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Financial Cost of Power Generation",
  "chunk_text": "\nDomestic coal based generation at pit head power plants is less expensive than \nnon-pit head coal based generation\n\n\nImported coal is not as competitive as domestic coal due to its higher costs\n\n\nDomestic gas based power generation is more expensive than domestic coal based \ngeneration at all locations\n\n\nAt locations away from the pithead, the difference between the cost of power \ngenerated using the two fuels reduces substantially. These are Delhi, Vadodara, \nKochi and Agartala\n\n\nLNG is the most expensive fuel for power generation due to the high costs of fuel \nand the prevailing exchan\n\n… [+72 more chars]",
  "content_hash": "e044832d28e672e26a95c3b9f890f7f9ee262996d9d0c18689b2421d99aa205b",
  "token_count": 161,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "34c69d7e-cc43-5579-9ded-b0bb9767afb8",
  "chunk_index": 4,
  "page_number": 8,
  "page_range": [
    8,
    9
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `fba43e57-4e0a-5543-97bc-b3c2906c7b17`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "fba43e57-4e0a-5543-97bc-b3c2906c7b17",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG\n\n| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 3.38 | 5.47 | 3.67 | 6.38 |\n| Bilaspur | 2.52 | 5.35 | 3.91 | 6.36 |\n| Vadodara | 3.18 | 5.16 | 3.60 | 6.31 |\n| Vishakapatnam | 2.86 | 4.93 | 3.55 | 6.12 |\n| Kochi | 3.85 | 5.10 | 3.97 | 6.12 |\n| Talcher | 2.52 | 4.98 | 3.91 | 6.33 |\n| Dhanbad | 2.68 | 5.09 | 3.95 | 6.38 |\n\n\n… [+79 more chars]",
  "content_hash": "d91685a4d8a62b4d00adddbf9263dff99b8dd14c22d76dfeab879261ff12bca5",
  "token_count": 306,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    9,
    9
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `de0b917e-94d3-52bf-9a9b-16cb106a43a4`

- vector: dim=3072 · [-0.0079, -0.0284, -0.0109, 0.0391, -0.0331, -0.0043, -0.0207, 0.0287, …]

```json
{
  "chunk_id": "de0b917e-94d3-52bf-9a9b-16cb106a43a4",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 3.38 | 5.47 | 3.67 | 6.38 |\n| Bilaspur | 2.52 | 5.35 | 3.91 | 6.36 |\n| Vadodara | 3.18 | 5.16 | 3.60 | 6.31 |\n| Vishakapatnam | 2.86 | 4.93 | 3.55 | 6.12 |\n| Kochi | 3.85 | 5.10 | 3.97 | 6.12 |\n| Talcher | 2.52 | 4.98 | 3.91 | 6.33 |\n| Dhanbad | 2.68 | 5.09 | 3.95 | 6.38 |\n| Agartala | 3.73 | 5.75 | 3.97 | 6.38 |\n| Nagpur | 3.52 | 5.35 | 3.63 | 6.28 |",
  "content_hash": "776ef50538fea8aabaa57cda3546a469683a9e0ce5cab0e470e9206376c57cb5",
  "token_count": 262,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "fba43e57-4e0a-5543-97bc-b3c2906c7b17",
  "chunk_index": 5,
  "page_number": 9,
  "page_range": [
    9,
    9
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `a888c55e-c7e3-5f34-a577-6b5d500adf8e`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "a888c55e-c7e3-5f34-a577-6b5d500adf8e",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Carbon Implications — Carbon Emissions and Power Generation",
  "chunk_text": "Carbon Implications — Carbon Emissions and Power Generation\n\n\nElectricity generation contributes the largest \nshare to carbon emissions in India \n\nCarbon is emitted in the process of extraction \nof resource and during burning of fuel during \npower production \n\nCarbon emitted in the process of coal based \npower generation is nearly twice that of gas \nbased generation \n\nThese costs are internalised while computing \nthe final cost of power generation \n\nThere are other impacts on the environment \ngenerated \nin \nthe \nprocess \nof \npower \nproduction but these are not incorporated in \nthe current\n\n… [+1465 more chars]",
  "content_hash": "7346c9e318b7c72af4fae325a7c45e2a58aa5cd87f99c2bc29b2e34232e4e3cc",
  "token_count": 540,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    11,
    13
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `8c08714e-4a44-5c35-a054-5d7fbb96d295`

- vector: dim=3072 · [0.0033, -0.0182, 0.0031, -0.0142, -0.0229, -0.0185, -0.0270, -0.0162, …]

```json
{
  "chunk_id": "8c08714e-4a44-5c35-a054-5d7fbb96d295",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Carbon Implications — Carbon Emissions and Power Generation",
  "chunk_text": "\nElectricity generation contributes the largest \nshare to carbon emissions in India \n\nCarbon is emitted in the process of extraction \nof resource and during burning of fuel during \npower production \n\nCarbon emitted in the process of coal based \npower generation is nearly twice that of gas \nbased generation \n\nThese costs are internalised while computing \nthe final cost of power generation \n\nThere are other impacts on the environment \ngenerated \nin \nthe \nprocess \nof \npower \nproduction but these are not incorporated in \nthe current exercise\n\nElectricity, \n715.83, 51% \nTransport, \n138.86, 10%\n\n… [+230 more chars]",
  "content_hash": "036f17ffbe582810f3f5ad9bf5d3adcf21422031032cc68af9ba87d4db7e5ab0",
  "token_count": 224,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "a888c55e-c7e3-5f34-a577-6b5d500adf8e",
  "chunk_index": 6,
  "page_number": 11,
  "page_range": [
    11,
    11
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `73703b23-20f5-59f8-bcba-2c2b3ec1657e`

- vector: dim=3072 · [-0.0023, -0.0226, 0.0057, 0.0499, -0.0020, -0.0308, -0.0425, 0.0155, …]

```json
{
  "chunk_id": "73703b23-20f5-59f8-bcba-2c2b3ec1657e",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Carbon Implications — Carbon Emissions and Power Generation",
  "chunk_text": "92, 9% \nIron and \nSteel, \n116.96, 9% \nOther \nindustries, \n158.98, 11% \nCO2 emissions distribution (million tonnes) across \nsectors in 2007 \nSource: Planning Commission , 2011 Calculating the Cost of Carbon \nThere has been significant volatility in carbon prices \nCurrent Carbon Emission Reduction (CER) prices in European Emission \nTrading Scheme (ETS) have been affected by prevailing macroeconomic \nand market situation and have declined substantially in 2011-12 \nPrices of carbon are based on the concept of Social Cost of Carbon – \nproposed by the Stern Review \nDifferent levels of social cos\n\n… [+747 more chars]",
  "content_hash": "dc67e989713e39e142d2d08e1c21a34a9109444bb78b972183d100f5167d8d40",
  "token_count": 364,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "a888c55e-c7e3-5f34-a577-6b5d500adf8e",
  "chunk_index": 7,
  "page_number": 12,
  "page_range": [
    12,
    13
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `b9011aca-a122-5da6-ba29-122d4603e6c9`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "b9011aca-a122-5da6-ba29-122d4603e6c9",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG\n\n| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 4.97 | 7.02 | 4.41 | 7.11 |\n| Bilaspur | 4.11 | 6.91 | 4.65 | 7.09 |\n| Vadodara | 4.76 | 6.72 | 4.34 | 7.04 |\n| Vishakapatnam | 4.45 | 6.48 | 4.29 | 6.86 |\n| Kochi | 5.44 | 6.65 | 4.72 | 6.86 |\n| Talcher | 4.11 | 6.53 | 4.65 | 7.07 |\n| Dhanbad | 4.26 | 6.64 | 4.70 | 7.11 |\n\n\n… [+244 more chars]",
  "content_hash": "51032f57548e312f0fb31126dc39c7c8f5c9b94814ab82b2452cecdff72497e8",
  "token_count": 360,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    13,
    14
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `67733f0d-abdb-5766-b5e5-b77a49577588`

- vector: dim=3072 · [0.0001, -0.0289, -0.0045, 0.0438, -0.0294, -0.0319, -0.0207, 0.0413, …]

```json
{
  "chunk_id": "67733f0d-abdb-5766-b5e5-b77a49577588",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 4.97 | 7.02 | 4.41 | 7.11 |\n| Bilaspur | 4.11 | 6.91 | 4.65 | 7.09 |\n| Vadodara | 4.76 | 6.72 | 4.34 | 7.04 |\n| Vishakapatnam | 4.45 | 6.48 | 4.29 | 6.86 |\n| Kochi | 5.44 | 6.65 | 4.72 | 6.86 |\n| Talcher | 4.11 | 6.53 | 4.65 | 7.07 |\n| Dhanbad | 4.26 | 6.64 | 4.70 | 7.11 |\n| Agartala | 5.32 | 7.30 | 4.72 | 7.11 |\n| Nagpur | 4.47 | 6.90 | 4.38 | 7.02 |\n\nAdding the Cost of Carbon to Power Generation Costs (2) — Total c\n\n… [+98 more chars]",
  "content_hash": "ac8953d393ed7951f04692a678dea29846a9f153e28f5c135fc1631b8aef0c7e",
  "token_count": 316,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "b9011aca-a122-5da6-ba29-122d4603e6c9",
  "chunk_index": 8,
  "page_number": 13,
  "page_range": [
    13,
    14
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `cb4e968c-f7f8-57f2-9300-756cb56fad26`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "cb4e968c-f7f8-57f2-9300-756cb56fad26",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG\n\n| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 3.91 | 5.98 | 3.92 | 6.62 |\n| Bilaspur | 3.05 | 5.87 | 4.15 | 6.60 |\n| Vadodara | 3.71 | 5.68 | 3.84 | 6.55 |\n| Vishakapatnam | 3.39 | 5.45 | 3.80 | 6.37 |\n| Kochi | 4.38 | 5.62 | 4.22 | 6.37 |\n| Talcher | 3.05 | 5.50 | 4.15 | 6.58 |\n| Dhanbad | 3.21 | 5.61 | 4.20 | 6.62 |\n\n\n… [+405 more chars]",
  "content_hash": "c62af5c968eb920644763335b387fa4387c088c81aaf78df4de51a6f1d021b5c",
  "token_count": 373,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    14,
    15
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `44219a8d-9d2b-5cba-82d4-3481586e68b4`

- vector: dim=3072 · [0.0020, -0.0184, -0.0136, 0.0486, -0.0440, -0.0095, -0.0251, 0.0205, …]

```json
{
  "chunk_id": "44219a8d-9d2b-5cba-82d4-3481586e68b4",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Delhi — Bilaspur — Vadodara — Vishakapatnam — Kochi — Talcher — Dhanbad — Agartala — Nagpur — Domestic Coal — Imported Coal — Domestic Gas — LNG",
  "chunk_text": "| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- |\n| Domestic Coal | Imported Coal | Domestic Gas | LNG |  |\n| Delhi | 3.91 | 5.98 | 3.92 | 6.62 |\n| Bilaspur | 3.05 | 5.87 | 4.15 | 6.60 |\n| Vadodara | 3.71 | 5.68 | 3.84 | 6.55 |\n| Vishakapatnam | 3.39 | 5.45 | 3.80 | 6.37 |\n| Kochi | 4.38 | 5.62 | 4.22 | 6.37 |\n| Talcher | 3.05 | 5.50 | 4.15 | 6.58 |\n| Dhanbad | 3.21 | 5.61 | 4.20 | 6.62 |\n| Agartala | 4.26 | 6.27 | 4.22 | 6.62 |\n| Nagpur | 3.41 | 5.87 | 3.88 | 6.53 |\n\nAdding the Cost of Carbon to Power Generation Cost (3) \nWhen the\n\n… [+259 more chars]",
  "content_hash": "4fe94da34efb9e4580445eacad79ecb1f9015ba3fc9bb0d3119beaca7e17ecee",
  "token_count": 329,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "cb4e968c-f7f8-57f2-9300-756cb56fad26",
  "chunk_index": 9,
  "page_number": 14,
  "page_range": [
    14,
    15
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `1d6cf2f5-006d-5b39-8370-11ba833d002f`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "1d6cf2f5-006d-5b39-8370-11ba833d002f",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "A Comparison",
  "chunk_text": "A Comparison\n\n| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- | --- | --- |\n| Imported Coal | Imported Coal | Imported Coal | LNG | LNG | LNG |  |\n| Without carbon cost | With carbon costs (at US$ 10) | With carbon costs (at US$ 30) | Without carbon cost | With carbon costs (at US$ 10) | With carbon costs (at US$ 30) |  |\n| Delhi | 5.47 | 5.98 | 7.02 | 6.38 | 6.62 | 7.11 |\n| Bilaspur | 5.35 | 5.87 | 6.91 | 6.36 | 6.60 | 7.09 |\n| Vadodara | 5.16 | 5.68 | 6.72 | 6.31 | 6.55 | 7.04 |\n| Vishakapatnam | 4.93\n\n… [+305 more chars]",
  "content_hash": "0fded77570c404f3d818082c04462f9e1d8630f79d4b78ef3de3f16815f05d66",
  "token_count": 424,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    16,
    16
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `4a62a11c-3875-5b45-9a61-ab362ce6bafc`

- vector: dim=3072 · [-0.0040, -0.0240, -0.0105, 0.0233, -0.0334, -0.0183, -0.0260, 0.0377, …]

```json
{
  "chunk_id": "4a62a11c-3875-5b45-9a61-ab362ce6bafc",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "A Comparison",
  "chunk_text": "| Plant Locations | Fuel Options | Fuel Options | Fuel Options | Fuel Options | Fuel Options | Fuel Options |\n| --- | --- | --- | --- | --- | --- | --- |\n| Imported Coal | Imported Coal | Imported Coal | LNG | LNG | LNG |  |\n| Without carbon cost | With carbon costs (at US$ 10) | With carbon costs (at US$ 30) | Without carbon cost | With carbon costs (at US$ 10) | With carbon costs (at US$ 30) |  |\n| Delhi | 5.47 | 5.98 | 7.02 | 6.38 | 6.62 | 7.11 |\n| Bilaspur | 5.35 | 5.87 | 6.91 | 6.36 | 6.60 | 7.09 |\n| Vadodara | 5.16 | 5.68 | 6.72 | 6.31 | 6.55 | 7.04 |\n| Vishakapatnam | 4.93 | 5.45 | 6.48\n\n… [+291 more chars]",
  "content_hash": "9cb687252dcf62a21fdc1fa1e00d8569c02a05e15a149cbc76ff5eb593b033f5",
  "token_count": 421,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "1d6cf2f5-006d-5b39-8370-11ba833d002f",
  "chunk_index": 10,
  "page_number": 16,
  "page_range": [
    16,
    16
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `490d8184-33f0-5e1a-87e0-7ef87ba5f4fc`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "490d8184-33f0-5e1a-87e0-7ef87ba5f4fc",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Domestic Coal and Gas Availability and Projected Requirements",
  "chunk_text": "Domestic Coal and Gas Availability and Projected Requirements\n\nDomestic Fuel Availability in India - \nCoal \nCoal and gas shortages in the country are constraining the development of \npower sector in the country\n\nThe reasons cited for shortages of coal are delay in obtaining clearances, \nbottlenecks in transport and evacuation facilities\n\nThe total requirement for non-coking coal is projected to increase to 913.3 \nMt (Million tonnes) by 2016-17 and further to 1268 Mt by 2021-22*\n\nEven if domestic production targets are met, the gap between requirement \nand domestic availability is projected\n\n… [+625 more chars]",
  "content_hash": "74e4e6a8e4eba492fe0c228d21e366d9373c62b6b74650448c544bb60004acef",
  "token_count": 299,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    18,
    19
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `ab72c2d8-8beb-5609-8848-69c1d776cc4e`

- vector: dim=3072 · [-0.0318, -0.0046, -0.0085, 0.0427, -0.0153, -0.0124, -0.0307, -0.0041, …]

```json
{
  "chunk_id": "ab72c2d8-8beb-5609-8848-69c1d776cc4e",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Domestic Coal and Gas Availability and Projected Requirements",
  "chunk_text": "Domestic Fuel Availability in India - \nCoal \nCoal and gas shortages in the country are constraining the development of \npower sector in the country\n\nThe reasons cited for shortages of coal are delay in obtaining clearances, \nbottlenecks in transport and evacuation facilities\n\nThe total requirement for non-coking coal is projected to increase to 913.3 \nMt (Million tonnes) by 2016-17 and further to 1268 Mt by 2021-22*\n\nEven if domestic production targets are met, the gap between requirement \nand domestic availability is projected to be 150 Mt in 2016-17 and 230 \nMt in in 2021-22*\n\n*Planning \n\n… [+562 more chars]",
  "content_hash": "9f3f99018fa9ecc16d40e9e6607c91512cb1e583108425761fadf0e2910191e2",
  "token_count": 288,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "490d8184-33f0-5e1a-87e0-7ef87ba5f4fc",
  "chunk_index": 11,
  "page_number": 18,
  "page_range": [
    18,
    19
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `de79e669-dbe6-5f6b-be36-5ebec19fdbbc`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "de79e669-dbe6-5f6b-be36-5ebec19fdbbc",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "International Fuel Markets — Changing Trends in International Markets",
  "chunk_text": "International Fuel Markets — Changing Trends in International Markets\n\n\nNeed to examine the international fuel \nmarkets and their evolution in the near to \nmedium term (5 to 10 years) \n\nDependence on international markets is \nrising for both coal and gas \n\nProduction of natural gas/shale oil and \ngas in North America – changing the flow \nof international energy trade \n\nNatural gas production in North America \n– likely to impact the fundamentals across \nEurope and Asia as well \n\nThis will have implications on the price of \nenergy \nsources \nin \nthe \ninternational \nmarkets \nPrice of Indian b\n\n… [+111 more chars]",
  "content_hash": "ba9759b0195a93279abac7e1cee09949ba39b2679d1d0779051beaa82d7cf462",
  "token_count": 191,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    21,
    21
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `838896fa-862b-5480-b74d-27c663b06e39`

- vector: dim=3072 · [-0.0522, -0.0368, -0.0083, 0.0250, -0.0106, -0.0276, -0.0086, 0.0246, …]

```json
{
  "chunk_id": "838896fa-862b-5480-b74d-27c663b06e39",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "International Fuel Markets — Changing Trends in International Markets",
  "chunk_text": "\nNeed to examine the international fuel \nmarkets and their evolution in the near to \nmedium term (5 to 10 years) \n\nDependence on international markets is \nrising for both coal and gas \n\nProduction of natural gas/shale oil and \ngas in North America – changing the flow \nof international energy trade \n\nNatural gas production in North America \n– likely to impact the fundamentals across \nEurope and Asia as well \n\nThis will have implications on the price of \nenergy \nsources \nin \nthe \ninternational \nmarkets \nPrice of Indian basket of crude oil \nMar-12\nApr-12\nMay-12\nJun-12\nJul-12\nAug-12\nSep-12\nOc\n\n… [+40 more chars]",
  "content_hash": "bd0ae47ff242f38b1fea1a6913e2ba9298c4607e6c288009d296d86be9e9241f",
  "token_count": 181,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "de79e669-dbe6-5f6b-be36-5ebec19fdbbc",
  "chunk_index": 12,
  "page_number": 21,
  "page_range": [
    21,
    21
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `b9ec61a5-def8-567b-a95f-b935a499b343`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "b9ec61a5-def8-567b-a95f-b935a499b343",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Global Coal Markets (1)",
  "chunk_text": "Global Coal Markets (1)\n\nCoal continues to be a major source of energy – fastest growing source in \nabsolute terms in 2011  \nEurope has witnessed an increase in coal consumption – primarily driven by \nswitching from natural gas to coal fired power generation, exports from USA to \nEurope \nIn the immediate term, coal consumption in Europe is expected to increase \nAs per the IEA – in the medium term – coal usage in OECD Europe will stagnate \nwhereas that in OECD Asia/Oceania will rise marginally \nCoal demand in USA has declined substantially – due to decline in gas prices \nand ageing coal fi\n\n… [+743 more chars]",
  "content_hash": "1c40c7bceeb8a34ec005d1a275ad1da5469132824aa706aa3384fb66f396db32",
  "token_count": 308,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    22,
    23
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `a3964f09-20d1-5522-894a-3cd3d038e329`

- vector: dim=3072 · [-0.0300, -0.0137, -0.0074, 0.0190, -0.0171, -0.0179, -0.0240, -0.0009, …]

```json
{
  "chunk_id": "a3964f09-20d1-5522-894a-3cd3d038e329",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Global Coal Markets (1)",
  "chunk_text": "Coal continues to be a major source of energy – fastest growing source in \nabsolute terms in 2011  \nEurope has witnessed an increase in coal consumption – primarily driven by \nswitching from natural gas to coal fired power generation, exports from USA to \nEurope \nIn the immediate term, coal consumption in Europe is expected to increase \nAs per the IEA – in the medium term – coal usage in OECD Europe will stagnate \nwhereas that in OECD Asia/Oceania will rise marginally \nCoal demand in USA has declined substantially – due to decline in gas prices \nand ageing coal fired power plants \nDemand\n\n… [+718 more chars]",
  "content_hash": "efcd0593d3dca5c4520ac2e459a081249336a4ef6e316b7664e7b7f89664d5ec",
  "token_count": 302,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "b9ec61a5-def8-567b-a95f-b935a499b343",
  "chunk_index": 13,
  "page_number": 22,
  "page_range": [
    22,
    23
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `ae28b633-d72f-5aab-9f5e-f6e6a166786a`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "ae28b633-d72f-5aab-9f5e-f6e6a166786a",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Changing policies in exporting nations ☐",
  "chunk_text": "Changing policies in exporting nations ☐\n\nD Increase in prices of coal in Indonesia and\n'potential' ban of certain exports\n\n[ Resource nationalism and sovereignty\n\n|  | Share in global coal reserves | R/P ratio |\n| --- | --- | --- |\n| % | # |  |\n| South Africa | 3.5 | 118 |\n| Australia | 8.9 | 184 |\n| Indonesia | 0.6 | 17 |\n| China | 13.3 | 33 |\n| India | 7 | 103 |\n\nSource: BP, 2012",
  "content_hash": "1ff6647cf5a9a646fb4ceedf9969dba7ef729f1794fee5f303454dcde24b4fb9",
  "token_count": 120,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    23,
    23
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `1501148f-f956-50ca-9833-1b0d159bd15c`

- vector: dim=3072 · [0.0020, -0.0208, -0.0061, 0.0014, -0.0334, -0.0029, -0.0032, 0.0055, …]

```json
{
  "chunk_id": "1501148f-f956-50ca-9833-1b0d159bd15c",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Changing policies in exporting nations ☐",
  "chunk_text": "D Increase in prices of coal in Indonesia and\n'potential' ban of certain exports\n\n[ Resource nationalism and sovereignty\n\n|  | Share in global coal reserves | R/P ratio |\n| --- | --- | --- |\n| % | # |  |\n| South Africa | 3.5 | 118 |\n| Australia | 8.9 | 184 |\n| Indonesia | 0.6 | 17 |\n| China | 13.3 | 33 |\n| India | 7 | 103 |\n\nSource: BP, 2012",
  "content_hash": "ffca27dd5e1df47b3effb3c7ae80bc350517ad6c6750425cebdbb3bce08f0b71",
  "token_count": 112,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "ae28b633-d72f-5aab-9f5e-f6e6a166786a",
  "chunk_index": 14,
  "page_number": 23,
  "page_range": [
    23,
    23
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `46b81129-c1f4-5aba-9304-9c3046e64050`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "46b81129-c1f4-5aba-9304-9c3046e64050",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Global Gas Markets",
  "chunk_text": "Global Gas Markets\n\n\n4 gas markets – North America, Europe,  Asia and Australia\n\n\nNatural gas prices in Asian markets - largely linked to crude oil prices\n\n\nProduction of shale gas in North America is altering the global gas markets\n\n\nUSA no longer dependent on imported LNG and is expected to now become an \nexporter\n\n\nLNG which was to be exported to USA will now be available to Europe and Asia \nAt lower rates?\n\n\nProgressive delinking of oil and gas prices in Europe – move towards gas on gas \ncompetition and pricing\n\n\nReports of gas contracts being renegotiated in these regions\n\n\nLikel\n\n… [+65 more chars]",
  "content_hash": "b40a322660ac028576b86e3a931c2a9ae7b8ee82dc2c31ed753bbd16da00a815",
  "token_count": 159,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    24,
    24
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `f9810318-41f6-50ea-b5d1-6e68f896d0e1`

- vector: dim=3072 · [-0.0506, -0.0139, -0.0184, 0.0532, -0.0255, -0.0210, -0.0025, 0.0132, …]

```json
{
  "chunk_id": "f9810318-41f6-50ea-b5d1-6e68f896d0e1",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Global Gas Markets",
  "chunk_text": "\n4 gas markets – North America, Europe,  Asia and Australia\n\n\nNatural gas prices in Asian markets - largely linked to crude oil prices\n\n\nProduction of shale gas in North America is altering the global gas markets\n\n\nUSA no longer dependent on imported LNG and is expected to now become an \nexporter\n\n\nLNG which was to be exported to USA will now be available to Europe and Asia \nAt lower rates?\n\n\nProgressive delinking of oil and gas prices in Europe – move towards gas on gas \ncompetition and pricing\n\n\nReports of gas contracts being renegotiated in these regions\n\n\nLikelihood of excess gas \n\n… [+45 more chars]",
  "content_hash": "95bda1bc6bfb1484b2cfd98ec91670e081c9dc74f04062831d8f65abd84cae59",
  "token_count": 155,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "46b81129-c1f4-5aba-9304-9c3046e64050",
  "chunk_index": 15,
  "page_number": 24,
  "page_range": [
    24,
    24
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `65209221-02ec-5b7b-b30f-e3905c4c06bb`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "65209221-02ec-5b7b-b30f-e3905c4c06bb",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Conclusions — Conclusions",
  "chunk_text": "Conclusions — Conclusions\n\nThe difference between cost of power generated using LNG and imported coal \nreduces if the carbon implications are taken into account \nIf the impact of increased shale gas availability in North America spreads to \nEurope and then to Asia, the rise in LNG prices may only be moderate \nThe tax etc. on coal exports from Australia and Indonesia are only likely to \nincrease,  making coal import costs higher \nThe climate change concerns and the National Action Plan on Climate Change \nalso warrant lowering emissions from power generation \nThe huge demand for fuel makes \n\n… [+784 more chars]",
  "content_hash": "01608a27d4516eae082ae926d2883adda4c24242d8d94ca78c216ba47edfdfaa",
  "token_count": 281,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    26,
    28
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `f3d0b003-0c5d-5024-b1b0-a9e7ee345a64`

- vector: dim=3072 · [-0.0319, -0.0073, -0.0046, 0.0185, -0.0241, -0.0118, -0.0482, 0.0155, …]

```json
{
  "chunk_id": "f3d0b003-0c5d-5024-b1b0-a9e7ee345a64",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Conclusions — Conclusions",
  "chunk_text": "The difference between cost of power generated using LNG and imported coal \nreduces if the carbon implications are taken into account \nIf the impact of increased shale gas availability in North America spreads to \nEurope and then to Asia, the rise in LNG prices may only be moderate \nThe tax etc. on coal exports from Australia and Indonesia are only likely to \nincrease,  making coal import costs higher \nThe climate change concerns and the National Action Plan on Climate Change \nalso warrant lowering emissions from power generation \nThe huge demand for fuel makes it imperative that we aband\n\n… [+757 more chars]",
  "content_hash": "313a335ab4163c264a36c811d6815dec686b5cb39d548b951fb707c67d4a4e27",
  "token_count": 275,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "65209221-02ec-5b7b-b30f-e3905c4c06bb",
  "chunk_index": 16,
  "page_number": 26,
  "page_range": [
    26,
    28
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Parent · `cf01a5e9-f230-560e-b247-ee562052d7cd`

- vector: dim=3072 · [0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, …]

```json
{
  "chunk_id": "cf01a5e9-f230-560e-b247-ee562052d7cd",
  "document_id": "es2012rs02_pdf",
  "is_parent": true,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Coal",
  "chunk_text": "Coal\n\n• Evacuation infrastructure in terms of rail networks to transport  \nmined coal to demand centres \nTransport infrastructure \n• Increasing the share of underground coal mining \nMining Technology \n• In-situ gasification is one of the most important ways to reach \ncoal at depths and to reduce the carbon impact \nUnderground coal \ngasification \n• Domestic coal is high in ash content \n• Current washery capacity for coal is only 33 Mt for coking \ncoal and 112 Mt for non-coking coal (2010-11, Coal Directory) \nWashery capacity \n• Port facilities \n• Rail links for in-land hauling of coal  \nImport \n\n… [+288 more chars]",
  "content_hash": "f24c960ad23de00a0ff04333e7648fd2a0ba30488ce8a84b394bf13331275b38",
  "token_count": 198,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "page_range": [
    29,
    30
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```

## Child · `9e5c0f3d-623b-54cd-8533-77b5a2d590f0`

- vector: dim=3072 · [-0.0256, 0.0194, -0.0057, -0.0047, -0.0282, -0.0245, -0.0383, 0.0161, …]

```json
{
  "chunk_id": "9e5c0f3d-623b-54cd-8533-77b5a2d590f0",
  "document_id": "es2012rs02_pdf",
  "is_parent": false,
  "source_type": "pdf",
  "title": "ES2012RS02.pdf",
  "section_heading": "Coal",
  "chunk_text": "• Evacuation infrastructure in terms of rail networks to transport  \nmined coal to demand centres \nTransport infrastructure \n• Increasing the share of underground coal mining \nMining Technology \n• In-situ gasification is one of the most important ways to reach \ncoal at depths and to reduce the carbon impact \nUnderground coal \ngasification \n• Domestic coal is high in ash content \n• Current washery capacity for coal is only 33 Mt for coking \ncoal and 112 Mt for non-coking coal (2010-11, Coal Directory) \nWashery capacity \n• Port facilities \n• Rail links for in-land hauling of coal  \nImport infras\n\n… [+282 more chars]",
  "content_hash": "0c79b2e2b51085fce7de9cfa0e2d730e25879d16fa1b30e16ee9dc8805231212",
  "token_count": 196,
  "doc_version": 1,
  "is_current": true,
  "tenant_id": "default",
  "acl": [
    "public"
  ],
  "language": "en",
  "pdf_id": "es2012rs02_pdf",
  "pdf_path": "ES2012RS02.pdf",
  "parent_chunk_id": "cf01a5e9-f230-560e-b247-ee562052d7cd",
  "chunk_index": 17,
  "page_number": 29,
  "page_range": [
    29,
    30
  ],
  "created_at": "2026-06-25T11:23:26.019224+00:00",
  "updated_at": "2026-06-25T11:23:26.019224+00:00"
}
```
