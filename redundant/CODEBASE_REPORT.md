...
- **Prompt tail:** `catalog_inventory_directive()` + `catalog_coverage_directive()` + `current_date_directive()`, in that order — corpus-stable blocks ahead of the daily-changing one, so the cacheable prefix stays maximal. The coverage block names the real `published_at` span (`catalog.queries.published_range`, TTL-cached, fails open) so the model neither scopes to a period the catalog cannot reach nor turns a bare "the latest" into a date bound: recency is ranking's job (§3.7), and a guessed bound would exclude the very documents that answer the question. The same tail rides on the structured planner and slot-extraction prompts.
